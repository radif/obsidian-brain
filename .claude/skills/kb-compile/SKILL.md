---
name: kb-compile
description: Use when the user wants to compile new or changed raw files into knowledge articles. Runs `uv run python scripts/compile.py`, which only processes files whose hash in the content repo's state.json changed since last run.
---
