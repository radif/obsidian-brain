...
Defaults: timestamps ON (each line `[m:ss] text` — better for citing moments later), language `en`, output saved to file. Flags available if the user asks: `--no-timestamps`, `--language=<code>`, `--stdout` (pipe instead of save).
